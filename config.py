...
CONF_DQN = {
    ...
    "batch_size": 64,
    "buffer_size": 1000000,
    "n_steps": 3,  # 2 steps is a regular TD(0)
    "init_sample_epsilon": .1,  # 1 means random sampling, for sampling before training
    "init_n_samples": 1000,
    "start_epsilon": .01,  # start for polynomial decay eps schedule, it should be real (double)
    "final_epsilon": .01,
...
CONF_RandomDQN = {
    ...
    "start_epsilon": .01,  # start for polynomial decay eps schedule, it should be real (double)
    "final_epsilon": .01,
    "optimizer": tf.keras.optimizers.Adam(lr=2.5e-6),
    "loss": tf.keras.losses.Huber(),
    "discount_rate": tf.constant(.99, dtype=tf.float32)
}
...
CONF_CategoricalDQN = {
    ...
    "batch_size": 64,
    "buffer_size": 1000000,
    "n_steps": 6,  # 2 steps is a regular TD(0)
    "init_sample_epsilon": .1,  # 1 means random sampling, for sampling before training
    "init_n_samples": 1000,
